...
def calc_force(a, b, dt,solar):
    ...
    F = ((G m_a m_b)/r^2)*((x_b-x_a)/r)
    """
    diff =  b[numpy.newaxis,:,1:4] - a[:,numpy.newaxis,1:4]

    r = numpy.sum(diff**2,axis = 2)**0.5
    if solar:
        r[numpy.diag_indices(r.shape[0])] = 1

    mass = G * a[:,numpy.newaxis,0]*b[numpy.newaxis,:,0]/(r**2)

    a[:,4] += numpy.sum(mass * (diff[:,:,0] / r) / a[:,numpy.newaxis,0]*dt, axis =1)
    a[:,5] += numpy.sum(mass * (diff[:,:,1] / r) / a[:,numpy.newaxis,0]*dt, axis =1)
    a[:,6] += numpy.sum(mass * (diff[:,:,2] / r) / a[:,numpy.newaxis,0]*dt, axis =1)
    #a[:,4:7] += numpy.sum(
    #    (G*a[:,numpy.newaxis,0]*b[numpy.newaxis,:,0]/(r**2))[:,:,numpy.newaxis] *
    #    (diff / r[:,:,numpy.newaxis] / a[:,numpy.newaxis,numpy.newaxis,0] * dt)
    #    ,axis = 1
    #)
