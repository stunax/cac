...
"""
This is an implementation of a matlab like ndgrid for numpy taken from: 

http://projects.scipy.org/numpy/ticket/966

It will probably be included in some future version of numpy, but we
manually add it for now.
"""
...
def meshgrid(*xi, **kwargs):
    ...
    ndim = len(args)
    s0 = (1, ) * ndim
    output = [x.reshape(s0[:i] + (-1, ) + s0[i + 1::]) for (i, x) in
              enumerate(args)]
